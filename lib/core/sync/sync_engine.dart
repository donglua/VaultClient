  ...
  Future<void> _pullRemote(
    ...
    try {
      ...
      if (relativePath.isEmpty) {
        ...
        final base = remoteBasePath.isEmpty
            ? ''
            : (remoteBasePath.endsWith('/')
                  ? remoteBasePath.substring(0, remoteBasePath.length - 1)
                  : remoteBasePath);
          ...
          if (await localFile.exists()) {
            ...
            if (file.mTime != null) {
              if (file.size == stat.size &&
                  (stat.modified.difference(file.mTime!).inSeconds).abs() < 2) {
